...
def main() -> int:
    ...
    p.add_argument("--start-m", default="200001")
    p.add_argument("--end-m", default="")
    p.add_argument("--start-q", default="2000Q1")
    ...
    p.add_argument("--datasets", default="index,index_basic,index_global,daily_info,futures,options,macro,fx")
    ...
    end_m = str(args.end_m) if args.end_m else end_date[:6]
    end_q = str(args.end_q) if args.end_q else f"{end_date[:4]}Q{((int(end_date[4:6]) - 1) // 3 + 1)}"
    ...
    if "index" in datasets:
        ...
        for s in (INDEX_DAILY, INDEX_DAILYBASIC, INDEX_WEEKLY):
            s.validate()
    ...
    if "daily_info" in datasets:
        ...
        for d in dates:
            for ex in ("SZ", "SH"):
                tasks.append(Task(spec=DAILY_INFO, request_params={"trade_date": d, "exchange": ex}))
    ...
    if "macro" in datasets:
        ...
        tasks.extend(period_builder.build(pro, MACRO_GDP, start=str(args.start_q), end=end_q))
        for s in (MACRO_CPI, MACRO_PPI, MACRO_M, MACRO_SF, MACRO_PMI):
            tasks.extend(period_builder.build(pro, s, start=str(args.start_m), end=end_m))

    if "futures" in datasets:
        from ..datasets.cffex_fut_daily import SPEC as FUT_DAILY
        from ..datasets.cffex_fut_mapping import SPEC as FUT_MAPPING

        for s in (FUT_DAILY, FUT_MAPPING):
            s.validate()

        fut_root = raw_root / "futures" / "cffex_fut_basic"
        df_basic = None
        if fut_root.exists():
            df_basic = _load_latest_snapshot_parquet(fut_root)
        fut_codes: list[str] = []
    ...
    if "options" in datasets:
        ...
        OPT_DAILY.validate()
        opt_root = raw_root / "options" / "cffex_opt_basic"
        df_opt = _load_latest_snapshot_parquet(opt_root)
        opt_codes: list[str] = []
    ...
    if "fx" in datasets:
        ...
        FX_DAILY.validate()
        fx_codes = sorted(set(universe.core_fx_symbols))
        code_builder = CodeRangeTaskBuilder(window="year")
